...
class ClusterManager<T extends ClusterItem> {
  ClusterManager(this._items, this.updateMarkers,
      {Future<map_interface.Marker> Function(Cluster<T>)? markerBuilder,
  ...
  final Future<map_interface.Marker> Function(Cluster<T>) markerBuilder;
  ...
  final void Function(Set<map_interface.Marker>) updateMarkers;
  ...
  void setMapId(int mapId, {bool withUpdate = true}) async {
    _mapId = mapId;
    _zoom = await map_interface.GoogleMapsFlutterPlatform.instance
  ...
  void _updateClusters() async {
    ...
    final Set<map_interface.Marker> markers =
  ...
  void onCameraMove(map_interface.CameraPosition position,
      {forceUpdate = false}) {
    _zoom = position.zoom;
    if (forceUpdate) {
      updateMap();
  ...
  Future<List<Cluster<T>>> getMarkers() async {
    ...
    final map_interface.LatLngBounds mapBounds = await map_interface
        .GoogleMapsFlutterPlatform.instance
        .getVisibleRegion(mapId: _mapId!);

    late map_interface.LatLngBounds inflatedBounds;
  ...
  map_interface.LatLngBounds _inflateBounds(map_interface.LatLngBounds bounds) {
    ...
    return map_interface.LatLngBounds(
      southwest: map_interface.LatLng(bounds.southwest.latitude - lat, wLng),
      northeast: map_interface.LatLng(
  ...
  static Future<map_interface.Marker> Function(Cluster)
      get _basicMarkerBuilder => (cluster) async {
            return map_interface.Marker(
              markerId: map_interface.MarkerId(cluster.getId()),
  ...
  static Future<map_interface.BitmapDescriptor> _getBasicClusterBitmap(int size,
      {String? text}) async {
    ...
    return map_interface.BitmapDescriptor.fromBytes(data.buffer.asUint8List());
  }
}
